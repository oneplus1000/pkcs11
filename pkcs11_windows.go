...
{	
	...
	if( list(&c->sym) != CKR_OK) {
		printf("error list(&c->sym) != CKR_OK ");
	}
	return c;
}
...
{
	CK_C_Finalize fun = (CK_C_Finalize) GetProcAddress(c->handle, "C_Finalize");
	return fun(NULL);
}
...
{	
	CK_C_GetSlotList slot = (CK_C_GetSlotList) GetProcAddress(c->handle, "C_GetSlotList");
	CK_RV e = slot(tokenPresent, NULL, ulCount);
	if (e != CKR_OK) {
		return e;
	}
	*slotList = calloc(*ulCount, sizeof(CK_SLOT_ID));
	e = slot(tokenPresent, *slotList, ulCount);
	return e;
}

CK_RV GetSlotInfo(struct ctx * c, CK_ULONG slotID, CK_SLOT_INFO_PTR info)
{
	CK_C_GetSlotInfo slotinfo = (CK_C_GetSlotInfo) GetProcAddress(c->handle, "C_GetSlotInfo");
	CK_RV e = slotinfo((CK_SLOT_ID) slotID, info);
	return e;
}
...
{	
	CK_C_OpenSession fun = (CK_C_OpenSession) GetProcAddress(c->handle, "C_OpenSession");
	CK_RV e =
	    fun((CK_SLOT_ID) slotID, (CK_FLAGS) flags, NULL,
				  NULL, session);
	return e;
}
...
{
	CK_C_CloseSession fun = (CK_C_CloseSession) GetProcAddress(c->handle, "C_CloseSession");
	CK_RV e = fun(session);
	return e;
}
	    ...
	    char *pin, CK_ULONG pinLen)
{
	...
	CK_C_Login fun = (CK_C_Login) GetProcAddress(c->handle, "C_Login");
	CK_RV e =
	    fun(session, userType, (CK_UTF8CHAR_PTR) pin, pinLen);
	return e;
}

CK_RV Logout(struct ctx * c, CK_SESSION_HANDLE session)
{
	CK_C_Logout fun = (CK_C_Logout) GetProcAddress(c->handle, "C_Logout");
	CK_RV e = fun(session);
	return e;
}
...
{
	CK_C_GetObjectSize fun = (CK_C_GetObjectSize) GetProcAddress(c->handle, "C_GetObjectSize");
	CK_RV e = fun(session, object, size);
	return e;
}
			...
			CK_ULONG templen)
{
	...
	CK_C_GetAttributeValue fun = (CK_C_GetAttributeValue) GetProcAddress(c->handle, "C_GetAttributeValue");
	CK_RV e = fun(session, object, tempc, templen);
	if (e != CKR_OK) {
		ATTR_FREE(tempc);
		return e;
	}
...
CK_RV SetAttributeValue(struct ctx * c, CK_SESSION_HANDLE session,
			...
			CK_ULONG templen)
{
	CK_C_SetAttributeValue fun = (CK_C_SetAttributeValue) GetProcAddress(c->handle, "C_SetAttributeValue");
	ATTR_TO_C(tempc, temp, templen, NULL)
	CK_RV e = fun(session, object, tempc, templen);
	ATTR_FREE(tempc);
	return e;
...
{
	ATTR_TO_C(tempc, temp, tempCount, NULL);
	CK_C_FindObjectsInit fun = (CK_C_FindObjectsInit) GetProcAddress(c->handle, "C_FindObjectsInit");
	CK_RV e = fun(session, tempc, tempCount);
	ATTR_FREE(tempc);
	return e;
}
		  ...
		  CK_ULONG_PTR objCount)
{
	*obj = calloc(max, sizeof(CK_OBJECT_HANDLE));
	CK_C_FindObjects fun = (CK_C_FindObjects) GetProcAddress(c->handle, "C_FindObjects");
	CK_RV e = fun(session, *obj, max, objCount);
	return e;
}

CK_RV FindObjectsFinal(struct ctx * c, CK_SESSION_HANDLE session)
{
	CK_C_FindObjectsFinal fun = (CK_C_FindObjectsFinal) GetProcAddress(c->handle, "C_FindObjectsFinal");
	CK_RV e = fun(session);
	return e;
}
...
{
	CK_C_EncryptInit fun = (CK_C_EncryptInit) GetProcAddress(c->handle, "C_EncryptInit");
	MECH_TO_C(m, mechanism);
	CK_RV e = fun(session, m, key);
	return e;
}
...
CK_RV DecryptInit(struct ctx * c, CK_SESSION_HANDLE session,
		  ckMechPtr mechanism, CK_OBJECT_HANDLE key)
{
	CK_C_DecryptInit fun = (CK_C_DecryptInit) GetProcAddress(c->handle, "C_DecryptInit");
	MECH_TO_C(m, mechanism);
	CK_RV e = fun(session, m, key);
	return e;
}
...
CK_RV DigestInit(struct ctx * c, CK_SESSION_HANDLE session,
		 ckMechPtr mechanism)
{
	CK_C_DigestInit fun = (CK_C_DigestInit) GetProcAddress(c->handle, "C_DigestInit");
	MECH_TO_C(m, mechanism);
	CK_RV e = fun(session, m);
	return e;
}
...
CK_RV SignInit(struct ctx * c, CK_SESSION_HANDLE session,
	       ckMechPtr mechanism, CK_OBJECT_HANDLE key)
{
	CK_C_SignInit fun = (CK_C_SignInit) GetProcAddress(c->handle, "C_SignInit");
	MECH_TO_C(m, mechanism);
	CK_RV e = fun(session, m, key);
	return e;
}
...
{
	CK_C_SignRecoverInit fun = (CK_C_SignRecoverInit) GetProcAddress(c->handle, "C_SignRecoverInit");
	MECH_TO_C(m, mech);
...
CK_RV VerifyInit(struct ctx * c, CK_SESSION_HANDLE session,
		 ckMechPtr mech, CK_OBJECT_HANDLE key)
{
	CK_C_VerifyInit fun = (CK_C_VerifyInit) GetProcAddress(c->handle, "C_VerifyInit");
	MECH_TO_C(m, mech);
	CK_RV rv = fun(session, m, key);
	return rv;
}
